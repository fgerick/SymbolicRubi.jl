...
@rule integrate((~a + ~b*ArcCoth(~c*~x))*(((~x)^-1)*(sqrt(~d + ~e*((~x)^2))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), EqQ(~e + ~d*((~c)^2), 0), GtQ(~d, 0)) 
 ~b*(sqrt(~d)^-1)*PolyLog(2, -(sqrt(1 + ~c*~x)^-1)*sqrt(1 - ~c*~x)) - ~b*(sqrt(~d)^-1)*PolyLog(2, (sqrt(1 + ~c*~x)^-1)*sqrt(1 - ~c*~x)) - 2(~a + ~b*ArcCoth(~c*~x))*(sqrt(~d)^-1)*ArcTanh((sqrt(1 + ~c*~x)^-1)*sqrt(1 - ~c*~x))
 end
...
@rule integrate(((~a + ~b*ArcTanh(~c*~x))^~p)*((~f*~x)^~m)*(sqrt(~d + ~e*((~x)^2))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), EqQ(~e + ~d*((~c)^2), 0), GtQ(~p, 0), LtQ(~m, -1), NeQ(~m, -2)) 
 ((~a + ~b*ArcTanh(~c*~x))^~p)*((~f*~x)^(1 + ~m))*(((~d)^-1)*((~f)^-1)*((1 + ~m)^-1))*sqrt(~d + ~e*((~x)^2)) + ((~c)^2)*(2 + ~m)*(((~f)^-2)*((1 + ~m)^-1))*integrate(((~a + ~b*ArcTanh(~c*~x))^~p)*((~f*~x)^(2 + ~m))*(sqrt(~d + ~e*((~x)^2))^-1), ~x) - ~b*~c*~p*(((~f)^-1)*((1 + ~m)^-1))*integrate(((~a + ~b*ArcTanh(~c*~x))^(~p - 1))*((~f*~x)^(1 + ~m))*(sqrt(~d + ~e*((~x)^2))^-1), ~x)
 end
