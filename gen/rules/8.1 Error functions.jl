...
@rule integrate(((~E)^(~c + ~d*((~x)^2)))*((~x)^~m)*Erfi(~a + ~b*~x), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), IGtQ(~m, 1)) 
 ((~E)^(~c + ~d*((~x)^2)))*((~x)^(~m - 1))*((1//2)*((~d)^-1))*Erfi(~a + ~b*~x) - ~b*(((~d)^-1)*(sqrt(~Pi)^-1))*integrate(((~E)^(~c + (~a)^2 + (~d + (~b)^2)*((~x)^2) + 2~a*~b*~x))*((~x)^(~m - 1)), ~x) - (~m - 1)*((1//2)*((~d)^-1))*integrate(((~E)^(~c + ~d*((~x)^2)))*((~x)^(~m - 2))*Erfi(~a + ~b*~x), ~x)
 end
...
@rule integrate(((~E)^(~c + ~d*((~x)^2)))*((~x)^~m)*Erfc(~a + ~b*~x), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), ILtQ(~m, -1)) 
 ((~E)^(~c + ~d*((~x)^2)))*((~x)^(1 + ~m))*((1 + ~m)^-1)*Erfc(~a + ~b*~x) + 2~b*(((1 + ~m)^-1)*(sqrt(~Pi)^-1))*integrate(((~E)^(~c - ((~a)^2) - ((~x)^2)*((~b)^2 - ~d) - 2~a*~b*~x))*((~x)^(1 + ~m)), ~x) - 2~d*((1 + ~m)^-1)*integrate(((~E)^(~c + ~d*((~x)^2)))*((~x)^(2 + ~m))*Erfc(~a + ~b*~x), ~x)
 end
