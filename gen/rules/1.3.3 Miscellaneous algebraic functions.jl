...
@rule integrate(~u*((~d*((~x)^~n) + ~c*sqrt(~a + ~b*((~x)^~p)))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~n), ~x), EqQ(~p, 2~n), EqQ(~b*((~c)^2) - ((~d)^2), 0)) 
 (((~a)^-1)*((~c)^-1))*integrate(~u*sqrt(~a + ~b*((~x)^(2~n))), ~x) - ~b*(((~a)^-1)*((~d)^-1))*integrate(~u*((~x)^~n), ~x)
 end

@rule integrate(((~x)^~m)*((~d*((~x)^~n) + ~c*sqrt(~a + ~b*((~x)^~p)))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~m, ~n), ~x), EqQ(~p, 2~n), NeQ(~b*((~c)^2) - ((~d)^2), 0)) 
...
@rule integrate(((~u + ~f*(~j + ~k*sqrt(~v)))^~n)*((~g + ~h*~x)^~m), ~x) =>  if And(FreeQ(List(~f, ~g, ~h, ~j, ~k, ~m, ~n), ~x), LinearQ(~u, ~x), QuadraticQ(~v, ~x), Not(And(LinearMatchQ(~u, ~x), QuadraticMatchQ(~v, ~x), Or(EqQ(~j, 0), EqQ(~f, 1)))), EqQ((~g*Coefficient(~u, ~x, 1) - ~h*(~f*~j + Coefficient(~u, ~x, 0)))^2 - (((~h)^2)*Coefficient(~v, ~x, 0) + ((~g)^2)*Coefficient(~v, ~x, 2) - ~g*~h*Coefficient(~v, ~x, 1))*((~f)^2)*((~k)^2), 0)) 
...
@rule integrate(((~g + ~h*~x)^~m)*((~e*~x + ~f*sqrt(~a + ~c*((~x)^2)))^~n), ~x) =>  if And(FreeQ(List(~a, ~c, ~e, ~f, ~g, ~h, ~n), ~x), EqQ((~e)^2 - ~c*((~f)^2), 0), IntegerQ(~m)) 
 (((~e)^(-1 - ~m))*(2^(-1 - ~m)))*Subst(integrate(((~x)^(~n - 2 - ~m))*((~h*((~x)^2) + 2~e*~g*~x - ~a*~h*((~f)^2))^~m)*(~a*((~f)^2) + (~x)^2), ~x), ~x, ~e*~x + ~f*sqrt(~a + ~c*((~x)^2)))
 end

@rule integrate(((~x)^~p)*((~g + ~i*((~x)^2))^~m)*((~e*~x + ~f*sqrt(~a + ~c*((~x)^2)))^~n), ~x) =>  if And(FreeQ(List(~a, ~c, ~e, ~f, ~g, ~i, ~n), ~x), EqQ((~e)^2 - ~c*((~f)^2), 0), EqQ(~c*~g - ~a*~i, 0), IntegersQ(~p, 2~m), Or(IntegerQ(~m), GtQ(~i*((~c)^-1), 0))) 
...
@rule integrate((((~x)^-1)*(sqrt(~c + ~d*((~x)^2))^-1))*sqrt(~a*((~x)^2) + ~b*~x*sqrt(~c + ~d*((~x)^2))), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), EqQ((~a)^2 - ~d*((~b)^2), 0), EqQ(~a + ~c*((~b)^2), 0)) 
 1.4142135623730951~b*((~a)^-1)*Subst(integrate(sqrt(1 + ((~a)^-1)*((~x)^2))^-1, ~x), ~x, ~a*~x + ~b*sqrt(~c + ~d*((~x)^2)))
 end
...
@rule integrate(((~c + ~d*~x)*sqrt(~a + ~b*((~x)^3)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), NeQ(((~b)^2)*((~c)^6) - 8((~a)^2)*((~d)^6) - 20~a*~b*((~c)^3)*((~d)^3), 0)) 
 With(List(Set(~q, Rt(~b*((~a)^-1), 3))), ~d*((2.732050807568877~d - ~c*~q)^-1)*integrate((2.732050807568877 + ~q*~x)*(((~c + ~d*~x)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)), ~x) - ~q*((2.732050807568877~d - ~c*~q)^-1)*integrate(sqrt(~a + ~b*((~x)^3))^-1, ~x))
 end
...
@rule integrate((~e + ~f*~x)*(((~c + ~d*~x)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ(~d*~e - ~c*~f, 0), EqQ(~b*((~e)^3) - 20.392304845413264~a*((~f)^3), 0), NeQ(~b*((~c)^3) + 0.39230484541326405~a*((~d)^3), 0)) 
 With(List(Set(~q, Simplify(2.732050807568877~f*((~e)^-1)))), 2.0705523608201664~f*(3^(4^-1))*(1 + ~q*~x)*(((~q)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)*(sqrt(((2.732050807568877 + ~q*~x)^-2)*(1 + ~q*~x))^-1))*sqrt((1 + ((~q)^2)*((~x)^2) - ~q*~x)*((2.732050807568877 + ~q*~x)^-2))*Subst(integrate(((~x*(2.732050807568877~d - ~c*~q) - 0.7320508075688772~d - ~c*~q)*sqrt(0.07179676972449123 + (~x)^2)*sqrt(1 - ((~x)^2)))^-1, ~x), ~x, ((2.732050807568877 + ~q*~x)^-1)*(0.7320508075688772 - ~q*~x)))
 end

@rule integrate((~e + ~f*~x)*(((~c + ~d*~x)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ(~d*~e - ~c*~f, 0), EqQ(~b*((~e)^3) + 0.39230484541326405~a*((~f)^3), 0), NeQ(~b*((~c)^3) - 20.392304845413264~a*((~d)^3), 0)) 
 With(List(Set(~q, Simplify(0.7320508075688772~f*((~e)^-1)))), 7.7274066103125465~f*(3^(4^-1))*(1 - ~q*~x)*(((~q)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)*(sqrt(((-0.7320508075688772 - ~q*~x)^-2)*(~q*~x - 1))^-1))*sqrt((1 + ~q*~x + ((~q)^2)*((~x)^2))*((-0.7320508075688772 - ~q*~x)^-2))*Subst(integrate(((2.732050807568877~d + ~c*~q + ~x*(~c*~q - 0.7320508075688772~d))*sqrt(1 - ((~x)^2))*sqrt(13.928203230275509 + (~x)^2))^-1, ~x), ~x, ((0.7320508075688772 + ~q*~x)^-1)*(2.732050807568877 - ~q*~x)))
 end
...
@rule integrate((~e + ~f*~x)*(((~c + ~d*~x)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ(~d*~e - ~c*~f, 0), NeQ(((~b)^2)*((~c)^6) - 8((~a)^2)*((~d)^6) - 20~a*~b*((~c)^3)*((~d)^3), 0), NeQ(((~b)^2)*((~e)^6) - 8((~a)^2)*((~f)^6) - 20~a*~b*((~e)^3)*((~f)^3), 0)) 
 With(List(Set(~q, Rt(~b*((~a)^-1), 3))), ((2.732050807568877~d - ~c*~q)^-1)*(2.732050807568877~f - ~e*~q)*integrate(sqrt(~a + ~b*((~x)^3))^-1, ~x) + ((2.732050807568877~d - ~c*~q)^-1)*(~d*~e - ~c*~f)*integrate((2.732050807568877 + ~q*~x)*(((~c + ~d*~x)^-1)*(sqrt(~a + ~b*((~x)^3))^-1)), ~x))
 end
...
@rule integrate(((~c + ~d*~x)*((~a + ~b*((~x)^3))^(3^-1)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), EqQ(~a*((~d)^3) + ~b*((~c)^3), 0)) 
 (((~c)^-1)*(2^((-7//1)*(1//3)))*(Rt(~b, 3)^-1))*Log((~c - ~d*~x)*((~c + ~d*~x)^2)) + 1.7320508075688772(((~c)^-1)*(2^((-4//1)*(1//3)))*(Rt(~b, 3)^-1))*ArcTan(0.5773502691896258(1 - (~c - ~d*~x)*(((~d)^-1)*((~a + ~b*((~x)^3))^(-(1//3))))*(2^(3^-1))*Rt(~b, 3))) - 3(((~c)^-1)*(2^((-7//1)*(1//3)))*(Rt(~b, 3)^-1))*Log((~c - ~d*~x)*Rt(~b, 3) + ~d*((~a + ~b*((~x)^3))^(3^-1))*(2^((2//1)*(1//3))))
 end
...
@rule integrate((~e + ~f*~x)*(((~c + ~d*~x)^-1)*((~a + ~b*((~x)^3))^(-(1//3)))), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), EqQ(~c*~f + ~d*~e, 0), EqQ(2~b*((~c)^3) - ~a*((~d)^3), 0)) 
 ~f*(((~d)^-1)*(Rt(~b, 3)^-1))*Log(~c + ~d*~x) + 1.7320508075688772~f*(((~d)^-1)*(Rt(~b, 3)^-1))*ArcTan(0.5773502691896258(1 + 2(2~c + ~d*~x)*(((~d)^-1)*((~a + ~b*((~x)^3))^(-(1//3))))*Rt(~b, 3))) - 3~f*((1//2)*((~d)^-1)*(Rt(~b, 3)^-1))*Log((2~c + ~d*~x)*Rt(~b, 3) - ~d*((~a + ~b*((~x)^3))^(3^-1)))
 end
...
@rule integrate(((~c + ~d*~x)*((~a + ~b*((~x)^3))^((2//1)*(1//3))))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), EqQ(2~b*((~c)^3) - ~a*((~d)^3), 0)) 
 With(List(Set(~q, Rt(~b, 3))), 3~d*((1//4)*((~c)^-2)*((~q)^-2))*Log(~q*(2~c + ~d*~x) - ~d*((~a + ~b*((~x)^3))^(3^-1))) + 1.7320508075688772~d*((1//2)*((~c)^-2)*((~q)^-2))*ArcTan(0.5773502691896258(1 + 2~q*(2~c + ~d*~x)*(((~d)^-1)*((~a + ~b*((~x)^3))^(-(1//3)))))) - ~d*((1//4)*((~c)^-2)*((~q)^-2))*Log(~q*~x - ((~a + ~b*((~x)^3))^(3^-1))) - ~d*(0.2886751345948129((~c)^-2)*((~q)^-2))*ArcTan(0.5773502691896258(1 + 2~q*~x*((~a + ~b*((~x)^3))^(-(1//3))))) - ~d*((1//2)*((~c)^-2)*((~q)^-2))*Log(~c + ~d*~x))
 end
