...
@rule integrate((~A + ~B*csc(~e + ~f*~x))*(Sqrt(~a + ~b*csc(~e + ~f*~x))^-1)*csc(~e + ~f*~x), ~x) =>  if And(FreeQ(List(~a, ~b, ~e, ~f, ~A, ~B), ~x), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~A)^2 - ((~B)^2), 0)) 
 (~A - ~B)*integrate((Sqrt(~a + ~b*Csc(~e + ~f*~x))^-1)*Csc(~e + ~f*~x), ~x) + ~B*integrate((Sqrt(~a + ~b*Csc(~e + ~f*~x))^-1)*(1 + Csc(~e + ~f*~x))*Csc(~e + ~f*~x), ~x)
 end
...
@rule integrate((~A + ~B*csc(~e + ~f*~x))*((~a + ~b*csc(~e + ~f*~x))^~m)*csc(~e + ~f*~x), ~x) =>  if And(FreeQ(List(~a, ~b, ~A, ~B, ~e, ~f), ~x), NeQ(~A*~b - ~B*~a, 0), NeQ((~a)^2 - ((~b)^2), 0), EqQ((~A)^2 - ((~B)^2), 0), Not(IntegerQ(2 * ~m))) 
 2 * ~A*(~A - ~B*Csc(~e + ~f*~x))*((~a + ~b*Csc(~e + ~f*~x))^~m)*(((~B)^-1)*((~f)^-1)*((~A*(~a + ~b*Csc(~e + ~f*~x))*((~A*~a + ~B*~b)^-1))^(-~m))*(cot(~e + ~f*~x)^-1))*AppellF1(2^-1, -(1//2), -~m, (3//1)*(1//2), (~A - ~B*Csc(~e + ~f*~x))*((1//2)*((~A)^-1)), ~b*(~A - ~B*Csc(~e + ~f*~x))*((~A*~b + ~B*~a)^-1))*Sqrt((~A + ~B*Csc(~e + ~f*~x))*((~A)^-1))*Sqrt(2)
 end
