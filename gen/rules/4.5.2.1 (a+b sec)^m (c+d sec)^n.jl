...
@rule integrate(((~c + ~d*csc(~e + ~f*~x))^-1)*Sqrt(~a + ~b*csc(~e + ~f*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ(~b*~c - ~a*~d, 0), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~c)^2 - ((~d)^2), 0)) 
 ((~c)^-1)*(~b*~c - ~a*~d)*integrate((((~c + ~d*Csc(~e + ~f*~x))^-1)*(Sqrt(~a + ~b*Csc(~e + ~f*~x))^-1))*Csc(~e + ~f*~x), ~x) + ~a*((~c)^-1)*integrate(Sqrt(~a + ~b*Csc(~e + ~f*~x))^-1, ~x)
 end
...
@rule integrate((Sqrt(~c + ~d*csc(~e + ~f*~x))^-1)*Sqrt(~a + ~b*csc(~e + ~f*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ(~b*~c - ~a*~d, 0), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~c)^2 - ((~d)^2), 0)) 
 2(~a + ~b*Csc(~e + ~f*~x))*(((~c)^-1)*((~f)^-1)*(cot(~e + ~f*~x)^-1)*(Rt((~a + ~b)*((~c + ~d)^-1), 2)^-1))*EllipticPi(~a*(~c + ~d)*(((~c)^-1)*((~a + ~b)^-1)), ArcSin((Sqrt(~a + ~b*Csc(~e + ~f*~x))^-1)*Sqrt(~c + ~d*Csc(~e + ~f*~x))*Rt((~a + ~b)*((~c + ~d)^-1), 2)), (~c + ~d)*(~a - ~b)*(((~a + ~b)^-1)*((~c - ~d)^-1)))*Sqrt((1 - Csc(~e + ~f*~x))*(~a*~d - ~b*~c)*(((~c + ~d)^-1)*((~a + ~b*Csc(~e + ~f*~x))^-1)))*Sqrt((~b*~c - ~a*~d)*(1 + Csc(~e + ~f*~x))*(((~c - ~d)^-1)*((~a + ~b*Csc(~e + ~f*~x))^-1)))
 end
