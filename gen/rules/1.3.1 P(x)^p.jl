...
@rule integrate((~a + ~b*~x + ~d*((~x)^3))^~p, ~x) =>  if And(FreeQ(List(~a, ~b, ~d), ~x), NeQ(4((~b)^3) + 27 * ~d*((~a)^2), 0), IntegerQ(~p)) 
 With(List(Set(~r, Rt(~d*Sqrt(3)*Sqrt(4 * ~d*((~b)^3) + 27((~a)^2)*((~d)^2)) - 9 * ~a*((~d)^2), 3))), ((~d)^(-2 * ~p))*integrate((Simp(~d*~x + ~b*~d*((1//3)*((~r)^-1))*(18^(3^-1)) - ~r*(18^(-(1//3))), ~x)^~p)*(Simp(((~d)^2)*((~x)^2) + ((~r)^2)*((1//3)*(12^(-(1//3)))) + (1//3)*~b*~d + ((~b)^2)*((~d)^2)*((1//3)*((~r)^-2))*(12^(3^-1)) - ~d*~x*(~b*~d*(((~r)^-1)*(3^(-(1//3))))*(2^(3^-1)) - ~r*(18^(-(1//3)))), ~x)^~p), ~x))
 end
