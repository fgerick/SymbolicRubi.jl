...
@rule integrate((~A + ~B*sin(~e + ~f*~x))*((Sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*(Sqrt(~c + ~d*sin(~e + ~f*~x))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f, ~A, ~B), ~x), EqQ(~b*~c + ~a*~d, 0), EqQ((~a)^2 - ((~b)^2), 0)) 
 (~A*~d + ~B*~c)*((1//2)*((~c)^-1)*((~d)^-1))*integrate((Sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*Sqrt(~c + ~d*sin(~e + ~f*~x)), ~x) + (~A*~b + ~B*~a)*((1//2)*((~a)^-1)*((~b)^-1))*integrate((Sqrt(~c + ~d*sin(~e + ~f*~x))^-1)*Sqrt(~a + ~b*sin(~e + ~f*~x)), ~x)
 end
...
@rule integrate((~A + ~B*sin(~e + ~f*~x))*(((~a + ~b*sin(~e + ~f*~x))^((-3//1)*(1//2)))*(Sqrt(~d*sin(~e + ~f*~x))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~d, ~e, ~f, ~A, ~B), ~x), NeQ((~a)^2 - ((~b)^2), 0)) 
 ~d*(((~a)^2 - ((~b)^2))^-1)*integrate((~A*~b + (~A*~a - ~B*~b)*sin(~e + ~f*~x) - ~B*~a)*(((~d*sin(~e + ~f*~x))^((-3//1)*(1//2)))*(Sqrt(~a + ~b*sin(~e + ~f*~x))^-1)), ~x) + (2 * ~A*~b - 2 * ~B*~a)*(((~f)^-1)*(((~a)^2 - ((~b)^2))^-1)*(Sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*(Sqrt(~d*sin(~e + ~f*~x))^-1))*cos(~e + ~f*~x)
 end
...
@rule integrate((~A + ~B*sin(~e + ~f*~x))*((~c + ~d*sin(~e + ~f*~x))^~n)*Sqrt(~a + ~b*sin(~e + ~f*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f, ~A, ~B), ~x), NeQ(~b*~c - ~a*~d, 0), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~c)^2 - ((~d)^2), 0), EqQ((~n)^2, 4^-1)) 
 ((3 + 2 * ~n)^-1)*integrate(((~c + ~d*sin(~e + ~f*~x))^(~n - 1))*(Sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*Simp(~B*(~b*~c + 2 * ~a*~d*~n) + (~B*(~a*~d + 2 * ~b*~c*~n) + ~A*~b*~d*(3 + 2 * ~n))*(sin(~e + ~f*~x)^2) + (~B*(~a*~c + ~b*~d)*(1 + 2 * ~n) + ~A*(3 + 2 * ~n)*(~b*~c + ~a*~d))*sin(~e + ~f*~x) + ~A*~a*~c*(3 + 2 * ~n), ~x), ~x) - 2 * ~B*((~c + ~d*sin(~e + ~f*~x))^~n)*(((~f)^-1)*((3 + 2 * ~n)^-1))*cos(~e + ~f*~x)*Sqrt(~a + ~b*sin(~e + ~f*~x))
 end
