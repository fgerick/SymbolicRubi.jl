...
@rule integrate((~d + ~e*((~x)^2))*(Sqrt(~a + ~c*((~x)^4))^-1), ~x) =>  if And(FreeQ(List(~a, ~c, ~d, ~e), ~x), LtQ(~a, 0), GtQ(~c, 0)) 
 With(List(Set(~q, Rt(-~a*~c, 2))), Condition(~e*~x*(~q + ~c*((~x)^2))*(((~c)^-1)*(Sqrt(~a + ~c*((~x)^4))^-1)) - ~e*~q*(((~c)^-1)*(Sqrt(~a + ~c*((~x)^4))^-1)*(Sqrt(-~a)^-1))*Sqrt(~q*((~x)^2) - ~a)*Sqrt((~a + ~q*((~x)^2))*((~q)^-1))*Sqrt(2)*EllipticE(ArcSin(~x*(Sqrt((~a + ~q*((~x)^2))*((1//2)*((~q)^-1)))^-1)), 2^-1), And(EqQ(~c*~d + ~e*~q, 0), IntegerQ(~q))))
 end

@rule integrate((~d + ~e*((~x)^2))*(Sqrt(~a + ~c*((~x)^4))^-1), ~x) =>  if And(FreeQ(List(~a, ~c, ~d, ~e), ~x), LtQ(~a, 0), GtQ(~c, 0)) 
 With(List(Set(~q, Rt(-~a*~c, 2))), Condition(~e*~x*(~q + ~c*((~x)^2))*(((~c)^-1)*(Sqrt(~a + ~c*((~x)^4))^-1)) - ~e*~q*(((~c)^-1)*(Sqrt(~a + ~c*((~x)^4))^-1)*(Sqrt(~a*((~a + ~q*((~x)^2))^-1))^-1))*Sqrt((~a + ~q*((~x)^2))*((~q)^-1))*Sqrt((~a - ~q*((~x)^2))*((~a + ~q*((~x)^2))^-1))*Sqrt(2)*EllipticE(ArcSin(~x*(Sqrt((~a + ~q*((~x)^2))*((1//2)*((~q)^-1)))^-1)), 2^-1), EqQ(~c*~d + ~e*~q, 0)))
 end
...
@rule integrate((~d + ~e*((~x)^2))*(Sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), GtQ((~b)^2 - 4 * ~a*~c, 0)) 
 With(List(Set(~q, Rt((~b)^2 - 4 * ~a*~c, 2))), Condition(~d*integrate(Sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4))^-1, ~x) + ~e*integrate(((~x)^2)*(Sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4))^-1), ~x), Or(PosQ(((~a)^-1)*(~b + ~q)), PosQ(((~a)^-1)*(~b - ~q)))))
 end
...
@rule integrate((Sqrt(~a + ~c*((~x)^4))*Sqrt(~d + ~e*((~x)^2)))^-1, ~x) =>  if And(FreeQ(List(~a, ~c, ~d, ~e), ~x), NeQ(~a*((~e)^2) + ~c*((~d)^2), 0)) 
 ((~x)^3)*((Sqrt(~a + ~c*((~x)^4))^-1)*(Sqrt(~d + ~e*((~x)^2))^-1))*Sqrt(~e + ~d*((~x)^-2))*Sqrt(~c + ~a*((~x)^-4))*integrate((((~x)^3)*Sqrt(~e + ~d*((~x)^-2))*Sqrt(~c + ~a*((~x)^-4)))^-1, ~x)
 end
...
@rule integrate((Sqrt(~d + ~e*((~x)^2))^-1)*Sqrt(~a + ~c*((~x)^4)), ~x) =>  if And(FreeQ(List(~a, ~c, ~d, ~e), ~x), NeQ(~a*((~e)^2) + ~c*((~d)^2), 0)) 
 (((~x)^-1)*(Sqrt(~d + ~e*((~x)^2))^-1)*(Sqrt(~c + ~a*((~x)^-4))^-1))*Sqrt(~a + ~c*((~x)^4))*Sqrt(~e + ~d*((~x)^-2))*integrate(~x*(Sqrt(~e + ~d*((~x)^-2))^-1)*Sqrt(~c + ~a*((~x)^-4)), ~x)
 end
