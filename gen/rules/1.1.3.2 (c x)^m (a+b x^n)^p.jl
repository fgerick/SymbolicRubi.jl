...
@rule integrate(((~x)^~m)*((~a + ~b*((~x)^~n))^-1), ~x) =>  if And(FreeQ(List(~a, ~b), ~x), IGtQ((1//4)*~n, 0), IGtQ(~m, 0), LtQ(~m, ~n - 1), GtQ(~a*((~b)^-1), 0)) 
 With(List(Set(~r, Numerator(Rt(~a*((~b)^-1), 4))), Set(~s, Denominator(Rt(~a*((~b)^-1), 4)))), ((~s)^3)*(0.35355339059327373((~b)^-1)*((~r)^-1))*integrate(((~x)^(~m - (1//4)*~n))*(((~r)^2 + ((~s)^2)*((~x)^((1//2)*~n)) - 1.4142135623730951~r*~s*((~x)^((1//4)*~n)))^-1), ~x) - ((~s)^3)*(0.35355339059327373((~b)^-1)*((~r)^-1))*integrate(((~x)^(~m - (1//4)*~n))*(((~r)^2 + ((~s)^2)*((~x)^((1//2)*~n)) + 1.4142135623730951~r*~s*((~x)^((1//4)*~n)))^-1), ~x))
 end
...
@rule integrate(~x*(sqrt(~a + ~b*((~x)^3))^-1), ~x) =>  if And(FreeQ(List(~a, ~b), ~x), PosQ(~a)) 
 With(List(Set(~r, Numer(Rt(~b*((~a)^-1), 3))), Set(~s, Denom(Rt(~b*((~a)^-1), 3)))), ((~r)^-1)*integrate((~r*~x - 0.7320508075688772~s)*(sqrt(~a + ~b*((~x)^3))^-1), ~x) + 0.7320508075688772~s*((~r)^-1)*integrate(sqrt(~a + ~b*((~x)^3))^-1, ~x))
 end

@rule integrate(~x*(sqrt(~a + ~b*((~x)^3))^-1), ~x) =>  if And(FreeQ(List(~a, ~b), ~x), NegQ(~a)) 
 With(List(Set(~r, Numer(Rt(~b*((~a)^-1), 3))), Set(~s, Denom(Rt(~b*((~a)^-1), 3)))), ((~r)^-1)*integrate((2.732050807568877~s + ~r*~x)*(sqrt(~a + ~b*((~x)^3))^-1), ~x) - 2.732050807568877~s*((~r)^-1)*integrate(sqrt(~a + ~b*((~x)^3))^-1, ~x))
 end
...
@rule integrate(((~x)^4)*(sqrt(~a + ~b*((~x)^6))^-1), ~x) =>  if FreeQ(List(~a, ~b), ~x) 
 With(List(Set(~r, Numer(Rt(~b*((~a)^-1), 3))), Set(~s, Denom(Rt(~b*((~a)^-1), 3)))), 0.7320508075688772((~s)^2)*((1//2)*((~r)^-2))*integrate(sqrt(~a + ~b*((~x)^6))^-1, ~x) - ((1//2)*((~r)^-2))*integrate((0.7320508075688772((~s)^2) - 2((~r)^2)*((~x)^4))*(sqrt(~a + ~b*((~x)^6))^-1), ~x))
 end
...
@rule integrate((((~a + ~b*((~x)^2))^(4^-1))*((~c*~x)^((3//1)*(1//2))))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c), ~x), PosQ(~b*((~a)^-1))) 
 (-2//1)*((~c)^-1)*((~a + ~b*((~x)^2))^(-(1//4)))*(sqrt(~c*~x)^-1) - ~b*((~c)^-2)*integrate(((~a + ~b*((~x)^2))^((-5//1)*(1//4)))*sqrt(~c*~x), ~x)
 end
...
@rule integrate((sqrt(~a + ~b*((~x)^2))^-1)*sqrt(~x), ~x) =>  if And(FreeQ(List(~a, ~b), ~x), GtQ(-~b*((~a)^-1), 0), GtQ(~a, 0)) 
 -2(((-~b*((~a)^-1))^((-3//1)*(1//4)))*(sqrt(~a)^-1))*Subst(integrate((sqrt(1 - ((~x)^2))^-1)*sqrt(1 - 2((~x)^2)), ~x), ~x, 0.7071067811865475sqrt(1 - ~x*sqrt(-~b*((~a)^-1))))
 end
...
@rule integrate(~x*((~a + ~b*((~x)^3))^((-2//1)*(1//3))), ~x) =>  if FreeQ(List(~a, ~b), ~x) 
 With(List(Set(~q, Rt(~b, 3))), -((1//2)*((~q)^-2))*Log(~q*~x - ((~a + ~b*((~x)^3))^(3^-1))) - (0.5773502691896258((~q)^-2))*ArcTan(0.5773502691896258(1 + 2~q*~x*((~a + ~b*((~x)^3))^(-(1//3))))))
 end
