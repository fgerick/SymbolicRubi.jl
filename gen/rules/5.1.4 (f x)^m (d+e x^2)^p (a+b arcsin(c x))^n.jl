...
@rule integrate(((~a + ~b*ArcCos(~c*~x))^~n)*((~f*~x)^~m)*Sqrt(~d + ~e*((~x)^2)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), EqQ(~e + ~d*((~c)^2), 0), GtQ(~n, 0), LtQ(~m, -1)) 
 ((~c)^2)*(((~f)^-2)*((1 + ~m)^-1))*Simp((Sqrt(1 - ((~c)^2)*((~x)^2))^-1)*Sqrt(~d + ~e*((~x)^2)))*integrate(((~a + ~b*ArcCos(~c*~x))^~n)*((~f*~x)^(2 + ~m))*(Sqrt(1 - ((~c)^2)*((~x)^2))^-1), ~x) + ((~a + ~b*ArcCos(~c*~x))^~n)*(((~f)^-1)*((1 + ~m)^-1))*((~f*~x)^(1 + ~m))*Sqrt(~d + ~e*((~x)^2)) + ~b*~c*~n*(((~f)^-1)*((1 + ~m)^-1))*Simp((Sqrt(1 - ((~c)^2)*((~x)^2))^-1)*Sqrt(~d + ~e*((~x)^2)))*integrate(((~a + ~b*ArcCos(~c*~x))^(~n - 1))*((~f*~x)^(1 + ~m)), ~x)
 end
...
@rule integrate(((~d + ~e*((~x)^2))^~p)*((~a + ~b*ArcCos(~c*~x))^~n)*((~f*~x)^~m), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), EqQ(~e + ~d*((~c)^2), 0), LtQ(~n, -1), IGtQ(2 * ~p, 0), NeQ(1 + ~m + 2 * ~p, 0), IGtQ(~m, -3)) 
 ~f*~m*(((~b)^-1)*((~c)^-1)*((1 + ~n)^-1))*Simp(((~d + ~e*((~x)^2))^~p)*((1 - ((~c)^2)*((~x)^2))^(-~p)))*integrate(((~a + ~b*ArcCos(~c*~x))^(1 + ~n))*((1 - ((~c)^2)*((~x)^2))^(~p - (1//2)))*((~f*~x)^(~m - 1)), ~x) - ((~d + ~e*((~x)^2))^~p)*((~a + ~b*ArcCos(~c*~x))^(1 + ~n))*((~f*~x)^~m)*(((~b)^-1)*((~c)^-1)*((1 + ~n)^-1))*Sqrt(1 - ((~c)^2)*((~x)^2)) - ~c*(1 + ~m + 2 * ~p)*(((~b)^-1)*((~f)^-1)*((1 + ~n)^-1))*Simp(((~d + ~e*((~x)^2))^~p)*((1 - ((~c)^2)*((~x)^2))^(-~p)))*integrate(((~a + ~b*ArcCos(~c*~x))^(1 + ~n))*((1 - ((~c)^2)*((~x)^2))^(~p - (1//2)))*((~f*~x)^(1 + ~m)), ~x)
 end
