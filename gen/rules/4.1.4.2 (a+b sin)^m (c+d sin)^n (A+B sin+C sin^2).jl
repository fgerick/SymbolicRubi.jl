...
@rule integrate((~A + ~C*(sin(~e + ~f*~x)^2) + ~B*sin(~e + ~f*~x))*((sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*(sqrt(~c + ~d*sin(~e + ~f*~x))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f, ~A, ~B, ~C), ~x), NeQ(~b*~c - ~a*~d, 0), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~c)^2 - ((~d)^2), 0)) 
 ((1//2)*((~d)^-1))*integrate((((~a + ~b*sin(~e + ~f*~x))^((-3//1)*(1//2)))*(sqrt(~c + ~d*sin(~e + ~f*~x))^-1))*Simp((2~B*~b*~d - ~C*(~b*~c + ~a*~d))*(sin(~e + ~f*~x)^2) + 2~A*~a*~d - ~C*(~b*~c - ~a*~d) - (2~C*~a*~c - 2~d*(~A*~b + ~B*~a))*sin(~e + ~f*~x), ~x), ~x) - ~C*(((~d)^-1)*((~f)^-1)*(sqrt(~a + ~b*sin(~e + ~f*~x))^-1))*cos(~e + ~f*~x)*sqrt(~c + ~d*sin(~e + ~f*~x))
 end
...
@rule integrate((~A + ~C*(sin(~e + ~f*~x)^2))*((sqrt(~a + ~b*sin(~e + ~f*~x))^-1)*(sqrt(~c + ~d*sin(~e + ~f*~x))^-1)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f, ~A, ~C), ~x), NeQ(~b*~c - ~a*~d, 0), NeQ((~a)^2 - ((~b)^2), 0), NeQ((~c)^2 - ((~d)^2), 0)) 
 ((1//2)*((~d)^-1))*integrate((((~a + ~b*sin(~e + ~f*~x))^((-3//1)*(1//2)))*(sqrt(~c + ~d*sin(~e + ~f*~x))^-1))*Simp(2~A*~a*~d - ~C*(~b*~c - ~a*~d) - (2~C*~a*~c - 2~A*~b*~d)*sin(~e + ~f*~x) - ~C*(~b*~c + ~a*~d)*(sin(~e + ~f*~x)^2), ~x), ~x) - ~C*(((~d)^-1)*((~f)^-1)*(sqrt(~a + ~b*sin(~e + ~f*~x))^-1))*cos(~e + ~f*~x)*sqrt(~c + ~d*sin(~e + ~f*~x))
 end
