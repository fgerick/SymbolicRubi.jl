...
@rule integrate((((~a + ~b*sin(~e + ~f*~x))^-1)*(sqrt(sin(~e + ~f*~x))^-1))*sqrt(~g*cos(~e + ~f*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~e, ~f, ~g), ~x), NeQ((~a)^2 - ((~b)^2), 0)) 
 -5.656854249492381~g*((~f)^-1)*Subst(integrate(((~x)^2)*((((~a + ~b)*((~g)^2) + (~a - ~b)*((~x)^4))^-1)*(sqrt(1 - ((~g)^-2)*((~x)^4))^-1)), ~x), ~x, (sqrt(1 + sin(~e + ~f*~x))^-1)*sqrt(~g*cos(~e + ~f*~x)))
 end
...
@rule integrate((((~a + ~b*sin(~e + ~f*~x))^-1)*(sqrt(cos(~e + ~f*~x))^-1))*sqrt(~d*sin(~e + ~f*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~d, ~e, ~f), ~x), NeQ((~a)^2 - ((~b)^2), 0)) 
 With(List(Set(~q, Rt((~b)^2 - ((~a)^2), 2))), 2.8284271247461903~d*(~b + ~q)*(((~f)^-1)*((~q)^-1))*Subst(integrate(((~a*((~x)^2) + ~d*(~b + ~q))*sqrt(1 - ((~d)^-2)*((~x)^4)))^-1, ~x), ~x, (sqrt(1 + cos(~e + ~f*~x))^-1)*sqrt(~d*sin(~e + ~f*~x))) - 2.8284271247461903~d*(~b - ~q)*(((~f)^-1)*((~q)^-1))*Subst(integrate(((~a*((~x)^2) + ~d*(~b - ~q))*sqrt(1 - ((~d)^-2)*((~x)^4)))^-1, ~x), ~x, (sqrt(1 + cos(~e + ~f*~x))^-1)*sqrt(~d*sin(~e + ~f*~x))))
 end
