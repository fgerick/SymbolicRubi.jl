...
@rule integrate(((~d + ~e*((~x)^4))^-1)*sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), EqQ(~c*~d + ~a*~e, 0), NegQ(~a*~c)) 
 With(List(Set(~q, sqrt((~b)^2 - 4~a*~c))), ~a*(0.35355339059327373((~d)^-1)*(Rt(-~a*~c, 2)^-1))*sqrt(~q - ~b)*ArcTanh(~x*(~b + ~q + 2~c*((~x)^2))*(0.35355339059327373(sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4))^-1)*(Rt(-~a*~c, 2)^-1))*sqrt(~q - ~b)) - ~a*(0.35355339059327373((~d)^-1)*(Rt(-~a*~c, 2)^-1))*sqrt(~b + ~q)*ArcTan(~x*(~b + 2~c*((~x)^2) - ~q)*(0.35355339059327373(sqrt(~a + ~b*((~x)^2) + ~c*((~x)^4))^-1)*(Rt(-~a*~c, 2)^-1))*sqrt(~b + ~q)))
 end
...
@rule integrate(((~e + ~f*~x)^~m)*((~a + ~b*~x + ~d*((~x)^3))^~p), ~x) =>  if And(FreeQ(List(~a, ~b, ~d, ~e, ~f, ~m), ~x), NeQ(4((~b)^3) + 27~d*((~a)^2), 0), ILtQ(~p, 0)) 
 With(List(Set(~r, Rt(1.7320508075688772~d*sqrt(4~d*((~b)^3) + 27((~a)^2)*((~d)^2)) - 9~a*((~d)^2), 3))), ((~d)^(-2~p))*integrate(((~e + ~f*~x)^~m)*(Simp(~d*~x + ~b*~d*((1//3)*((~r)^-1))*(18^(3^-1)) - ~r*(18^(-(1//3))), ~x)^~p)*(Simp(((~d)^2)*((~x)^2) + ((~r)^2)*((1//3)*(12^(-(1//3)))) + (1//3)*~b*~d + ((~b)^2)*((~d)^2)*((1//3)*((~r)^-2))*(12^(3^-1)) - ~d*~x*(~b*~d*(((~r)^-1)*(3^(-(1//3))))*(2^(3^-1)) - ~r*(18^(-(1//3)))), ~x)^~p), ~x))
 end
...
@rule integrate(~x*(sqrt(~a + ~b*~x + ~c*((~x)^2) + ~e*((~x)^4))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~e), ~x), EqQ(71((~c)^2) + 100~a*~e, 0), EqQ(1152((~c)^3) - 125~e*((~b)^2), 0)) 
 With(List(Set(~Px, (1//320)*(40~e*((~a)^2) + 6~a*((~c)^2) + 33~c*((~b)^2)) + ((~e)^3)*((~x)^8) + 2~c*((~e)^2)*((~x)^6) + (1//4)*~e*((~x)^4)*(5((~c)^2) + 4~a*~e) + 4*(1//3)*~b*((~e)^2)*((~x)^5) + 22*(1//15)*~b*~c*~e*((~x)^3) - 22*(1//5)*~a*~c*~e*((~x)^2))), ((1//8)*(Rt(~e, 2)^-1))*Log(~Px + sqrt(~a + ~b*~x + ~c*((~x)^2) + ~e*((~x)^4))*Dist((8~x*Rt(~e, 2))^-1, D(~Px, ~x), ~x)))
 end
