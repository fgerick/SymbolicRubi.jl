...
@rule integrate((sqrt(~a + ~b*~x + ~c*((~x)^2))*sqrt(~d + ~e*~x + ~f*((~x)^2)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e, ~f), ~x), NeQ((~b)^2 - 4~a*~c, 0), NeQ((~e)^2 - 4~d*~f, 0)) 
 With(List(Set(~r, Rt((~b)^2 - 4~a*~c, 2))), (sqrt(~a + ~b*~x + ~c*((~x)^2))^-1)*sqrt(2~a + ~x*(~b + ~r))*sqrt(~b + ~r + 2~c*~x)*integrate((sqrt(2~a + ~x*(~b + ~r))*sqrt(~b + ~r + 2~c*~x)*sqrt(~d + ~e*~x + ~f*((~x)^2)))^-1, ~x))
 end

@rule integrate((sqrt(~d + ~f*((~x)^2))*sqrt(~a + ~b*~x + ~c*((~x)^2)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~f), ~x), NeQ((~b)^2 - 4~a*~c, 0)) 
 With(List(Set(~r, Rt((~b)^2 - 4~a*~c, 2))), (sqrt(~a + ~b*~x + ~c*((~x)^2))^-1)*sqrt(2~a + ~x*(~b + ~r))*sqrt(~b + ~r + 2~c*~x)*integrate((sqrt(~d + ~f*((~x)^2))*sqrt(2~a + ~x*(~b + ~r))*sqrt(~b + ~r + 2~c*~x))^-1, ~x))
 end
