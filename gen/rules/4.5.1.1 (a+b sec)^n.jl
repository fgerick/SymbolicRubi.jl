...
@rule integrate(sqrt(~a + ~b*csc(~c + ~d*~x)), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), NeQ((~a)^2 - ((~b)^2), 0)) 
 2(~a + ~b*Csc(~c + ~d*~x))*(((~d)^-1)*(cot(~c + ~d*~x)^-1)*(Rt(~a + ~b, 2)^-1))*sqrt(~b*((~a + ~b*Csc(~c + ~d*~x))^-1)*(1 + Csc(~c + ~d*~x)))*sqrt(-~b*((~a + ~b*Csc(~c + ~d*~x))^-1)*(1 - Csc(~c + ~d*~x)))*EllipticPi(~a*((~a + ~b)^-1), ArcSin((sqrt(~a + ~b*Csc(~c + ~d*~x))^-1)*Rt(~a + ~b, 2)), (~a - ~b)*((~a + ~b)^-1))
 end
...
@rule integrate(sqrt(~a + ~b*csc(~c + ~d*~x))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d), ~x), NeQ((~a)^2 - ((~b)^2), 0)) 
 2(((~a)^-1)*((~d)^-1)*(cot(~c + ~d*~x)^-1))*sqrt(~b*((~a + ~b)^-1)*(1 - Csc(~c + ~d*~x)))*sqrt(-~b*((~a - ~b)^-1)*(1 + Csc(~c + ~d*~x)))*EllipticPi((~a + ~b)*((~a)^-1), ArcSin((Rt(~a + ~b, 2)^-1)*sqrt(~a + ~b*Csc(~c + ~d*~x))), (~a + ~b)*((~a - ~b)^-1))*Rt(~a + ~b, 2)
 end
