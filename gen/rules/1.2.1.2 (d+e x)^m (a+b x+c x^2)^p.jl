...
@rule integrate(((~d + ~e*~x)*((~a + ~b*~x + ~c*((~x)^2))^(3^-1)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), NeQ(2 * ~c*~d - ~b*~e, 0), EqQ(((~c)^2)*((~d)^2) + ((~b)^2)*((~e)^2) - 3 * ~a*~c*((~e)^2) - ~b*~c*~d*~e, 0), PosQ(~c*((~e)^2)*(2 * ~c*~d - ~b*~e))) 
 With(List(Set(~q, Rt(3 * ~c*((~e)^2)*(2 * ~c*~d - ~b*~e), 3))), 3 * ~c*~e*((1//2)*((~q)^-2))*Log(~c*~d - ~b*~e - ~q*((~a + ~b*~x + ~c*((~x)^2))^(3^-1)) - ~c*~e*~x) - 3 * ~c*~e*((1//2)*((~q)^-2))*Log(~d + ~e*~x) - ~c*~e*((~q)^-2)*ArcTan(Sqrt(3)^-1 + 2(~c*~d - ~b*~e - ~c*~e*~x)*(((~q)^-1)*((~a + ~b*~x + ~c*((~x)^2))^(-(1//3)))*(Sqrt(3)^-1)))*Sqrt(3))
 end

@rule integrate(((~d + ~e*~x)*((~a + ~c*((~x)^2))^(3^-1)))^-1, ~x) =>  if And(FreeQ(List(~a, ~c, ~d, ~e), ~x), EqQ(~c*((~d)^2) - 3 * ~a*((~e)^2), 0)) 
 With(List(Set(~q, Rt(6((~c)^2)*((~d)^-2)*((~e)^2), 3))), 3 * ~c*~e*((1//2)*((~d)^-2)*((~q)^-2))*Log(~c*~d - ~c*~e*~x - ~d*~q*((~a + ~c*((~x)^2))^(3^-1))) - 3 * ~c*~e*((1//2)*((~d)^-2)*((~q)^-2))*Log(~d + ~e*~x) - ~c*~e*(((~d)^-2)*((~q)^-2))*ArcTan(Sqrt(3)^-1 + 2 * ~c*(~d - ~e*~x)*(((~d)^-1)*((~q)^-1)*((~a + ~c*((~x)^2))^(-(1//3)))*(Sqrt(3)^-1)))*Sqrt(3))
 end
...
@rule integrate(((~d + ~e*~x)*((~a + ~b*~x + ~c*((~x)^2))^(3^-1)))^-1, ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), NeQ(2 * ~c*~d - ~b*~e, 0), EqQ(((~c)^2)*((~d)^2) + ((~b)^2)*((~e)^2) - 3 * ~a*~c*((~e)^2) - ~b*~c*~d*~e, 0), NegQ(~c*((~e)^2)*(2 * ~c*~d - ~b*~e))) 
 With(List(Set(~q, Rt(-3 * ~c*((~e)^2)*(2 * ~c*~d - ~b*~e), 3))), 3 * ~c*~e*((1//2)*((~q)^-2))*Log(~c*~d + ~q*((~a + ~b*~x + ~c*((~x)^2))^(3^-1)) - ~b*~e - ~c*~e*~x) - 3 * ~c*~e*((1//2)*((~q)^-2))*Log(~d + ~e*~x) - ~c*~e*((~q)^-2)*ArcTan(Sqrt(3)^-1 - 2(~c*~d - ~b*~e - ~c*~e*~x)*(((~q)^-1)*((~a + ~b*~x + ~c*((~x)^2))^(-(1//3)))*(Sqrt(3)^-1)))*Sqrt(3))
 end
