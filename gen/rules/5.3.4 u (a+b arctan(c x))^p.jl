...
@rule integrate((~a + ~b*ArcCot(~c*~x))*(Sqrt(~d + ~e*((~x)^2))^-1), ~x) =>  if And(FreeQ(List(~a, ~b, ~c, ~d, ~e), ~x), EqQ(~e, ~d*((~c)^2)), GtQ(~d, 0)) 
 ~I*~b*(((~c)^-1)*(Sqrt(~d)^-1))*PolyLog(2, ~I*(Sqrt(1 - ~I*~c*~x)^-1)*Sqrt(1 + ~I*~c*~x)) - ~I*~b*(((~c)^-1)*(Sqrt(~d)^-1))*PolyLog(2, -~I*(Sqrt(1 - ~I*~c*~x)^-1)*Sqrt(1 + ~I*~c*~x)) - 2 * ~I*(~a + ~b*ArcCot(~c*~x))*(((~c)^-1)*(Sqrt(~d)^-1))*ArcTan((Sqrt(1 - ~I*~c*~x)^-1)*Sqrt(1 + ~I*~c*~x))
 end
